	...
	try {
		...
		for (let name in global.plugins) {
			...
			if ((usedPrefix = (match[0] || '')[0])) {
				...
				if (
	m.isGroup &&
	global.db.data.chats[m.chat]?.antibot &&
	plugin.type !== 'protection'
) continue;
				//انتهاء اضافة antibot
				m.plugin = name;
...
export async function participantsUpdate({ id, participants, action, simulate = false }) {
	...
	switch (action) {
		case 'add':
		case 'remove':
			// Welcome/Goodbye is now handled entirely by dedicated plugins (e.g. welcome.js)
			// via the plugin.participantsUpdate hook, instead of the old inline chat.welcome logic.
			for (let name in global.plugins) {
				let plugin = global.plugins[name];
				if (!plugin || plugin.disabled) continue;
				if (typeof plugin.participantsUpdate !== 'function') continue;
				try {
					await plugin.participantsUpdate.call(this, { id, participants, action, groupMetadata, simulate });
				} catch (e) {
					console.error(e);
				}
			}
